...
lstm = Sequential() #import generic model
lstm.add(LSTM(units=10, activation='tanh', recurrent_activation='sigmoid')) 
lstm.add(Dense(2, activation='tanh')) #Regular BPNN layer   
lstm.add(Dropout(0.2)) #Optional, dropping randomly some nodes to prevent overfitting
lstm.add(Dense(1)) #Finish with 1 since we expect one output
lstm.compile(loss='mse')

#Fit the model
lstm.fit(X_train, Y_train, epochs = 5, batch_size = 1, verbose=0)

#Printing model summary
print(lstm.summary())

#Prediction
Y_train_predict = lstm.predict(X_train)
